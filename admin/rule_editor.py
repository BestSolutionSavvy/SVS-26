...
ui.add_css('html, body, #q-app { height: 100%; overflow: hidden !important; margin: 0; } #q-app > div { height: 100%; overflow: hidden !important; } .mermaid-preview { width: 100%; overflow: auto; } .mermaid-preview svg { display: block; margin: 0 auto !important; }')
...
def render_rules_list() -> None:
    ...
    with rules_list_container:
        ...
        for rule_name in rules_list:
            ui.button(rule_name).on_click(lambda r=rule_name: handle_rule_click(r)).classes('w-full justify-start mb-2')
...
def save_current_rule() -> None:
    ...
    rules_dir.mkdir(parents=True, exist_ok=True)
    saved_path = mermaid_to_yaml_file(content['code'], current_rule_path, default_title=current_rule_name)
    ui.notify(f'Saved {saved_path.name}')
    render_rules_list()

def update_chart():
    """Update the Mermaid chart with current content"""
    global chart
    if chart:
        chart.content = content['code']

def load_rule(filename):
    """Load a YAML rule file and update the editor"""
    ...
    with ui.column().classes('w-full overflow-auto'):
        with ui.row().classes('w-full items-center justify-between mb-2'):
            ...
            with ui.row().classes('items-center gap-1'):
                ui.button(icon='add', on_click=open_new_rule_dialog).props('flat round dense').tooltip('New rule')
                ui.button(icon='close', on_click=close_rules_drawer).props('flat round dense')
...
with ui.row().classes('w-full no-wrap h-screen overflow-hidden'):
    with ui.column().classes('w-1/2 pt-12 h-full flex flex-col overflow-hidden'):
        with ui.row().classes('w-full items-center justify-between mb-2'):
            ui.label('Editor Mermaid').classes('text-h6')
            ui.button(icon='save', color='secondary', on_click=save_current_rule).props('unelevated')
    ...
    with ui.column().classes('w-1/2 pt-4 h-full flex items-center justify-center overflow-hidden border-l border-gray-300'):
        with ui.column().classes('w-full h-full flex items-center justify-center overflow-hidden'):
            ui.label('Preview').classes('text-h6')

            chart = ui.mermaid(content['code']).classes('mermaid-preview').style('min-height: 75vh; max-width: 95%;')

        editor.on_value_change(lambda _: update_chart())

ui.run(native=True, title='Admin Rule Editor', window_size=(1200, 700), favicon='favicon.ico')