...
class Notification:
    def __init__(self, title: str, type: StateType, duration: float = 3.0):
        ...
        duration: float
            Duration in seconds of the notification (default 3.0)
        """
        self.title = title
        self.type = type
    ...
    def get_visibility(self) -> float:
        ...
        if elapsed > self.duration * 0.8:
            visibility = 1 - (elapsed - self.duration *
                              0.8) / (self.duration * 0.2)
        else:
            visibility = 1.0
...
class HudDrawer:
    """Handles drawing of HUD elements like violation notifications."""
    ...
    def _draw_notification_box(self, notification: Notification, y: int, alpha: int):
        ...
        if is_violation:
            pygame.draw.circle(surf, (*accent, 255),
                               (icon_cx, icon_cy), ICON_SZ // 2)
