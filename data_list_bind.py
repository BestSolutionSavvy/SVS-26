...
class VariableBinder:
    ...
    def __init__(self, world: carla.World, ego_vehicle: carla.Actor,
        ...
        self.map_obj = world.get_map()  # cached once, never changes
    ...
    def get_distance_to_lead(self, vehicles: Optional[List] = None) -> float:
    ...
    def get_abs_lane_deviation(self, map_obj: Optional[carla.Map] = None) -> float:
    ...
    def is_in_intersection(self, map_obj: Optional[carla.Map] = None) -> bool:
    ...
    def get_lane_marking_type(self, map_obj: Optional[carla.Map] = None) -> str:
    ...
    def is_vehicle_on_right(self, narrow: bool = False,
                             vehicles: Optional[List] = None) -> bool:
    ...
    def compute_scene_data(self) -> Dict[str, Any]:
        ...
        Costly RPC calls to the CARLA server are made only once per frame:
        - world.get_actors() called once, result shared across all methods
        - world.get_map() cached in __init__, never re-fetched
        - get_lane_marking_type() called once, result reused for both flags
        """
        # One RPC call for all vehicles, shared across methods
        all_vehicles = list(self.world.get_actors().filter("vehicle.*"))

        # One map query for all waypoint-based methods (map_obj cached in __init__)
        lane_marking = self.get_lane_marking_type(map_obj=self.map_obj)
        ...
        return {
            ...
            # SAFE_DISTANCE rule
            'distance_to_lead': self.get_distance_to_lead(vehicles=all_vehicles),
            ...
            'in_intersection': self.is_in_intersection(map_obj=self.map_obj),
            'precedence': self.has_right_of_way(),
            'broad_right_occupied': self.is_vehicle_on_right(narrow=False, vehicles=all_vehicles),
            'narrow_right_occupied': self.is_vehicle_on_right(narrow=True, vehicles=all_vehicles),

            # LANE_KEEPING rule
            'abs_lane_deviation': self.get_abs_lane_deviation(map_obj=self.map_obj),
