...
def test_state_machine_transition_on_true_condition(dummy_rule, idle_state, warning_state):
    ...
    sm = StateMachine(dummy_rule)
    new_state, transition, scene_data = sm.evaluate(LazyDict({"value": 11}))
    assert new_state == warning_state, f"Expected to transition to warning_state, got {new_state}"
...
def test_state_machine_transition_back_on_false_condition(dummy_rule, idle_state, warning_state):
    ...
    new_state, transition, scene_data = sm.evaluate(LazyDict({"value": 9}))  # go back to idle
...
def test_state_machine_stays_in_state_on_no_match(dummy_rule, idle_state):
    ...
    new_state, transition, scene_data = sm.evaluate(LazyDict({"value": 10}))
